			...
			for (int roty = 1; roty < 180; roty++) {
				for (int rotz = 1; rotz < 180; rotz++) {
					for (int a = 0; a < ordered.length; a++) {
						...
						float b = ((ordered[a].getX() * ordered[a].getX()) / (res.length.length() * res.length.length())) + ((ordered[a].getY() * ordered[a].getY()) / (res.width.length() * res.width.length())) + ((ordered[a].getZ() * ordered[a].getZ()) / (res.height.length() * res.height.length()));
						if (b == 0) {
