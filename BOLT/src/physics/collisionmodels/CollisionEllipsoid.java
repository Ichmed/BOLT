...
public class CollisionEllipsoid {
	 ...
	 */
	public CollisionEllipsoid(Vector3f middle, float length, float width,
	 ...
	 *            the third radius
	 */
	public CollisionEllipsoid(float x, float y, float z, float length,
	...
	public static CollisionEllipsoid createCollisionEllipsoid(
			Vector3f... points) {
		CollisionSphere temp = CollisionSphere.createCollisionSphere(points);
		CollisionEllipsoid res = new CollisionEllipsoid(temp.middle,
				temp.radius * 2, temp.radius * 2, temp.radius * 2);
		...
		for (int i = 0; i < points.length; i++) {
			...
			abstaende[i] = (float) Math.sqrt(buffer.getX() * buffer.getX()
					+ buffer.getY() * buffer.getY() + buffer.getZ()
					* buffer.getZ());
		}
			...
			for (int roty = 1; roty < 180; roty++) {
				for (int rotz = 1; rotz < 180; rotz++) {
					for (int a = 0; a < ordered.length; a++) {
						abstaende[a] = MathHelper
								.calculateDistancePointToPoint(res.middle,
										ordered[a]);
						float b = ((ordered[a].getX() * ordered[a].getX()) / (res.length * res.length))
								+ ((ordered[a].getY() * ordered[a].getY()) / (res.width * res.width))
