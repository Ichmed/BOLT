...
/**
 * Provides a text-field that makes suggestions using a provided data-vector. You might have seen this on Google (tm), this is the Java implementation.
 * 
 * @author David von Ah
 * @version 0.5
 */
public class JSuggestField extends JTextField
{
	...
	/**
	 * Separate matcher-thread, prevents the text-field from hanging while the suggestions are beeing prepared.
	 */
	...
	/**
	 * Inner class providing the independent matcher-thread. This thread can be interrupted, so it won't process older requests while there's already a new one.
	 */
	 ...
	 *            matcher that determines if a data word may be suggested for the current search word.
	 */
