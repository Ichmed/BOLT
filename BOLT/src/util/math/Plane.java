...
public class Plane {
	...
	public Vector3f intersectWithLine(Line line) {
		...
		if (Vector3f.dot(line.getDirection(), this.normal) == 0) return null;
		this.transformToHesseNormalForm();
		// Calculates the factor for the direction-vector of the line
		float factor = (Vector3f.dot(this.normal, this.startingPoint) - Vector3f.dot(line.getDirection(), line.getStartingPoint())) / Vector3f.dot(line.getDirection(), this.normal);
	...
	public Vector3f[] intersectWithParabola(Parabola par) {
		this.transformToHesseNormalForm();
		float discriminant = Vector3f.dot(normal, par.getDirection()) * Vector3f.dot(normal, par.getDirection()) - 4 * Vector3f.dot(normal, par.getInfluence()) * (Vector3f.dot(normal, par.getStartingPoint()) - Vector3f.dot(normal, startingPoint));
		if (discriminant < 0) {
	...
	public Line intersectWithPlane(Plane plane) {
		...
		if (plane.getNormal().x == this.getNormal().x && plane.getNormal().y == this.getNormal().y && plane.getNormal().z == this.getNormal().z) return null;
		...
		// just for backup: -->
		if (matrix[0][0] == 0 || matrix[1][1] == 0 || matrix[2][2] == 0) return null;
		if (matrix[1][0] != 0) {
